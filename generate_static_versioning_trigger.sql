...
BEGIN
  ...
  IF %2$L != 'tstzrange' THEN
    RAISE 'system period column %% does not have type tstzrange', %3$L;
  END IF;
  IF %4$L != 'tstzrange' THEN
    RAISE 'history system period column %% does not have type tstzrange', %3$L;
  END IF;

  IF %5$L AND TG_OP = 'UPDATE' AND (%6$s) IS NOT DISTINCT FROM (%7$s) THEN
    RETURN OLD;
  END IF;

  IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' OR (%8$L AND TG_OP = 'INSERT') THEN
    existing_range := OLD.%3$I;
    IF existing_range IS NULL THEN
      RAISE 'system period column %% must not be null', %3$L;
    END IF;
    IF isempty(existing_range) OR NOT upper_inf(existing_range) THEN
      RAISE 'system period column %% contains invalid value', %3$L;
    END IF;
    ...
    IF %8$L THEN
      IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
        UPDATE %9$I SET %3$I = tstzrange(range_lower, time_stamp_to_use, '[)')
        WHERE (%10$s) = (%10$s) AND %3$I = OLD.%3$I;
      END IF;
      IF TG_OP = 'UPDATE' OR TG_OP = 'INSERT' THEN
        INSERT INTO %9$I (%10$s, %3$I) VALUES (%6$s, tstzrange(time_stamp_to_use, NULL, '[)'));
      END IF;
    ELSE
      INSERT INTO %9$I (%10$s, %3$I) VALUES (%7$s, tstzrange(range_lower, time_stamp_to_use, '[)'));
    END IF;
  END IF;

  IF TG_OP = 'UPDATE' OR TG_OP = 'INSERT' THEN
    NEW.%3$I := tstzrange(time_stamp_to_use, NULL, '[)');
    RETURN NEW;
  END IF;
...
$outer$,
  ...
  sys_period_type,             -- 2
  p_sys_period,                -- 3
  history_sys_period_type,     -- 4
  p_ignore_unchanged_values,   -- 5
  new_row_compare,             -- 6
  old_row_compare,             -- 7
  p_include_current_version_in_history, -- 8
  p_history_table,             -- 9
  common_columns               -- 10
);
